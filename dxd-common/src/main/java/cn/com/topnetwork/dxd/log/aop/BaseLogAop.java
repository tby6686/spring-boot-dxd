...
package cn.com.topnetwork.dxd.log.aop;
...
import cn.com.topnetwork.dxd.constant.properties.SpringBootAopProperties;
import cn.com.topnetwork.dxd.log.annotation.Module;
import cn.com.topnetwork.dxd.log.annotation.OperationLog;
import cn.com.topnetwork.dxd.log.annotation.OperationLogIgnore;
import cn.com.topnetwork.dxd.log.bean.ClientInfo;
import cn.com.topnetwork.dxd.log.bean.OperationLogInfo;
import cn.com.topnetwork.dxd.log.bean.RequestInfo;
...
import cn.com.topnetwork.dxd.util.*;
...
public abstract class BaseLogAop {
     ...
     * Aop日志配置
     */
    protected SpringBootAopProperties.LogAopConfig logAopConfig;

    /**
     * 日志打印类型
     */
    protected SpringBootAopProperties.LogPrintType logPrintType;
     ...
     * requestId生成类型
     */
    protected SpringBootAopProperties.RequestIdType requestIdType;

    /**
     * Aop操作日志配置
     */
    protected SpringBootAopProperties.OperationLogConfig operationLogConfig;

    /**
     * Aop登录日志配置
     */
    protected SpringBootAopProperties.LoginLogConfig loginLogConfig;

    @Autowired
    public void setSpringBootAopProperties(SpringBootAopProperties springBootPlusAopProperties) {
    ...
    public Object handle(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            // 用户浏览器代理字符串
            requestInfo.setUserAgent(request.getHeader(ConstantCommon.USER_AGENT));
    ...
    protected void handleRequestId(RequestInfo requestInfo) {
       ...
       /* if (SpringBootAopProperties.RequestIdType.IDWORK == requestIdType) {
            requestId = IdWorker.getIdStr();
        } else if (SpringBootAopProperties.RequestIdType.UUID == requestIdType) {
    ...
    protected void handleRequestInfo(RequestInfo requestInfo) {
        ...
        if (SpringBootAopProperties.LogPrintType.NONE == logPrintType) {
            return;
        }
        // 获取请求信息
        String requestInfoString = formatRequestInfo(requestInfo);
        // 如果打印方式为顺序打印，则直接打印，否则，保存的threadLocal中
        if (SpringBootAopProperties.LogPrintType.ORDER == logPrintType) {
    ...
    protected String formatRequestInfo(RequestInfo requestInfo) {
        ...
        try {
            if (logAopConfig.isRequestLogFormat()) {
                requestInfoString = "\n" + JacksonUtil.toJsonStringNonNull(requestInfo, true);
            } else {
                requestInfoString = JacksonUtil.toJsonStringNonNull(requestInfo);
            }
        } catch (Exception e) {
    ...
    protected void handleResponseResult(Object result) {
        ...
        if (result != null && result instanceof ApiResult) {
            ...
            if (SpringBootAopProperties.LogPrintType.ORDER == logPrintType) {
                ...
                if (SpringBootAopProperties.LogPrintType.LINE == logPrintType) {
                    printRequestInfoString(requestInfoString);
                    printResponseResult(code, responseResultString);
                } else if (SpringBootAopProperties.LogPrintType.MERGE == logPrintType) {
    ...
    protected String formatResponseResult(ApiResult<?> apiResult) {
        ...
        try {
            if (logAopConfig.isResponseLogFormat()) {
                responseResultString += "\n" + JacksonUtil.toJsonString(apiResult, true);
            } else {
                responseResultString += JacksonUtil.toJsonString(apiResult);
            }
            int code = apiResult.getCode();
    ...
    protected void saveSysOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            // 设置请求参数信息
            if (requestInfo != null) {
                sysOperationLog.setIp(requestInfo.getIpAddress())
                ...
                // 设置参数字符串
                sysOperationLog.setParam(JacksonUtil.toJsonStringNonNull(requestInfo.getParam()));
                // User-Agent
                ...
                // 设置IP区域
                requestInfo.setIpAddress(requestInfo.getIpAddress());
            }
            ...
            sysOperationLog.setUserId(Long.valueOf(JwtUtil.getUserId(token))).setUserName(JwtUtil.getUsername(token));
            ...
            if (exception != null) {
                ...
                if (exception instanceof BaseException) {
                    BaseException springBootException = (BaseException) exception;
                    errorCode = springBootException.getErrorCode();
                }
                // 异常字符串长度截取
    ...
    protected void saveSysLoginLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            // 保存登录登出日志
            if (flag) {
                SysLoginLogVo sysLoginLog = new SysLoginLogVo();
                ...
                if (result != null && result instanceof ApiResult) {
                    ...
                    if (apiResult.isSuccess()) {
                        if (LOGIN_TYPE == type) {
                            ...
                            if (object != null) {
                                // 设置登录token
                                String tokenMd5 = DigestUtils.md5Hex(object.toString());
                ...
                // 设置请求参数信息
                if (requestInfo != null) {
                    sysLoginLog.setIp(requestInfo.getIpAddress()).setRequestId(requestInfo.getRequestId());
                    // 设置登录用户名
                    if (LOGIN_TYPE == type) {
                        Object paramObject = requestInfo.getParam();
                        if (paramObject != null && paramObject instanceof LoginParam) {
                            LoginParam loginUsername = (LoginParam) paramObject;
                    ...
                    sysLoginLog.setIp(requestInfo.getIpAddress());
                    // 保存登录日志
